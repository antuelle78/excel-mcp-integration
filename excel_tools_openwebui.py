...
class Tools:
    ...
    def __init__(self):
        # MCP server and file server configuration
        ...
        self.file_server_url = os.getenv(
            "FILE_SERVER_URL", "http://localhost:8001"
        )  # For file downloads (port 8001, per src/main.py)
        self.headers = {
        ...
        try:
            init_payload = {
                ...
                "params": {
                    ...
                    "capabilities": {"tools": {}},
                    "clientInfo": {"name": "openwebui-excel-tools", "version": "1.0.0"},
                },
            }

            with httpx.Client(timeout=10.0) as client:
                response = client.post(
                    self.mcp_server_url, json=init_payload, headers=self.headers
                )
    ...
    def _call_mcp_tool(self, tool_name: str, **kwargs) -> str:
        ...
        try:
            ...
            with httpx.Client(timeout=30.0) as client:
                ...
                if response.headers.get("content-type") == "text/event-stream":
                    ...
                    if result_data and "result" in result_data:
                        return self._process_result_with_file_access(
                            result_data["result"]
                        )
            ...
            if (
                "session" in str(e.response.text).lower()
                or "missing" in str(e.response.text).lower()
            ...
            if isinstance(item, dict) and item.get("type") == "text":
                ...
                if (
                    ...
                    if file_match:
                        ...
                        file_path = (
                            file_match.group(0)
                            if "output/" in file_match.group(0)
    ...
    async def create_sales_report(
        self, filename: str, sales_data: List[List[Any]], include_chart: bool = True
    ) -> str:
    ...
    async def create_employee_directory(
        self, filename: str, employee_data: List[List[Any]]
    ) -> str:
                            ...
                            if line_str.startswith("data: "):
                                ...
                                if data:
                                    ...
                                    if (
                                        ...
                                        and "tools" in result_data["result"]
                                    ):
    ...
    def create_excel_workbook(
        ...
        try:
            ...
            if not sheet_data:
                return {
                    "success": False,
                    "error": f"Sheet '{sheet_name}' is empty - no data provided",
                }
            ...
            if isinstance(sheet_data, dict):
                ...
                if not sheet_data:
                    return {
                        "success": False,
                        "error": "List data is empty - please provide at least headers or data",
                    }
                ...
                if isinstance(sheet_data[0], dict):
                    if not sheet_data[0]:
                        return {
                            "success": False,
                            "error": "First dictionary in list is empty",
                        }
                    ...
                    if not sheet_data[0]:
                        return {
                            "success": False,
                            "error": "First row (headers) is empty",
                        }
                ...
                return {
                    "success": False,
                    "error": f"Unsupported data format: {type(sheet_data).__name__}. Expected dict, list of dicts, or list of lists.",
                }
            ...
            if not headers:
                return {
                    "success": False,
                    "error": "No headers could be extracted from the data",
                }

            if not rows:
                return {
                    "success": False,
                    "error": "No data rows could be extracted from the data",
                }
            ...
            for i, row in enumerate(rows):
                if len(row) != len(headers):
                    return {
                        "success": False,
                        "error": f"Row {i + 1} has {len(row)} columns but headers have {len(headers)} columns",
                    }
            ...
            return {
                ...
                "download_link": (
                    f"{self.file_server_url}/files/{filename}"
                    if "Successfully created" in result
                    else None
                ),
            }
            ...
            return {
                "success": False,
                "error": f"Failed to create Excel workbook: {str(e)}",
            }
